...
void ArrayList_ArrayList2(ArrayList * pArray, int * c, int length)
{
	pArray->size = length;
	pArray->capacity = length;
	pArray->data = (int *)malloc(sizeof(int) * length);
	...
	memcpy(pArray->data, c, length * sizeof(int));
}
...
bool DefaultGroovyMethods_retainAll(HashSet * self, int * item, int length)
{
	ArrayList * pArray = (ArrayList *)malloc(sizeof(pArray));
	ArrayList_ArrayList2(pArray, item, length);

	bool bResult = HashSet_retainAll(self, pArray);
...
int main(int argc, char ** argv)
{
	...
	//HashMap_print(set->map);

	//printf("\n");

	DefaultGroovyMethods_retainAll(set, array, size);
