...
void ListEntry_ListEntry(ListEntry * pEntry, MapEntry * element, ListEntry * next, ListEntry * previous)
{
...
ListEntry * LinkedList_addBefore( LinkedList * pList,  MapEntry * e, ListEntry * entry)
{
...
bool LinkedList_add(LinkedList * pList , MapEntry * e)
{
	LinkedList_addBefore(pList, e, pList->header);
	return true;
}

void LinkedList_addLast(LinkedList * pList, MapEntry * e)
{
	LinkedList_addBefore(pList, e, pList->header);
}
...
int LinkedList_indexOf(LinkedList * pList, MapEntry * o)
{
	ListEntry * e;
	int index = 0;
...
bool LinkedList_contains(LinkedList * pList, MapEntry * o)
{
	return LinkedList_indexOf(pList, o);
}


typedef struct stDualHashBidiMap
{
	HashMap * normalMap;
	HashMap * reverseMap;
} DualHashBidiMap;

void DualHashBidiMap_DualHashBidiMap( DualHashBidiMap * pMap )
{
	pMap->normalMap = (HashMap *)malloc(sizeof(HashMap));
	HashMap_HashMap0(pMap->normalMap);

	pMap->reverseMap = (HashMap *)malloc(sizeof(HashMap));
	HashMap_HashMap0(pMap->reverseMap);
	
}

void DualHashBidiMap_destroy(DualHashBidiMap * pMap )
{
	HashMap_destroy(pMap->normalMap);
	free(pMap->normalMap);

	HashMap_destroy(pMap->reverseMap);
	free(pMap->reverseMap);
}

void DualHashBidiMap_put(DualHashBidiMap * pMap, int key, int value)
{
	if(HashMap_containsKey(pMap->normalMap, key))
	{
		HashMap_remove(pMap->reverseMap, HashMap_get(pMap->normalMap, key));
	}

	if(HashMap_containsKey(pMap->reverseMap, value) )
	{
		HashMap_remove(pMap->normalMap, HashMap_get(pMap->reverseMap, value));
	}

	HashMap_put(pMap->normalMap, key, value);
	HashMap_put(pMap->reverseMap, value, key);
}

void DualHashBidiMap_removeAll(DualHashBidiMap * pMap, LinkedList * coll)
{
	bool modified = false;
	int i ;

	for(i = 0; i < pMap->normalMap->capacity; i ++ )
	{
		MapEntry * current = pMap->normalMap->table[i];
		MapEntry * prev = current;
		MapEntry * n = current;
		...
		while(current != NULL)
		{
			...
			if(LinkedList_contains(coll, current))
			{
				if(prev == current)
				{
					pMap->normalMap->table[i] = n;
					prev = n;
				}
				else
				{
				...
				HashMap_remove(pMap->reverseMap, current->value);
				free(current);
				modified = true;
				current = n;
...
int main(int argc, char ** argv)
{
	...
	int i ;
	DualHashBidiMap * bidi = (DualHashBidiMap *)malloc(sizeof(DualHashBidiMap));
	DualHashBidiMap_DualHashBidiMap(bidi);

	for(i = 0; i < size; i ++)
	{
		DualHashBidiMap_put(bidi, i, i);
	}

	HashMap * map = (HashMap *)malloc(sizeof(HashMap));
	HashMap_HashMap0(map);
	...
	LinkedList_LinkedList(toRemove);

	for(i = 0; i < map->capacity; i ++ )
	{
		MapEntry * current = map->table[i];

		while(current != NULL)
		{
			LinkedList_add(toRemove, current);
			current = current->next;
		}
	}

	DualHashBidiMap_removeAll(bidi, toRemove);

	//HashMap_print(bidi->normalMap);
	//HashMap_print(bidi->reverseMap);

	LinkedList_destroy(toRemove);
	free(toRemove);
	DualHashBidiMap_destroy(bidi);
	free(bidi);
}
