...
void ListOrderedSet_ListOrderedSet(ListOrderedSet * pOrderedSet)
{
	pOrderedSet->setOrder = (ArrayList *)malloc(sizeof(ArrayList));
	ArrayList_ArrayList0(pOrderedSet->setOrder);

	pOrderedSet->collection = (HashSet *)malloc(sizeof(HashSet));
	HashSet_HashSet(pOrderedSet->collection);

}

void ListOrderedSet_destroy( ListOrderedSet * pOrderedSet)
{
	ArrayList_destroy(pOrderedSet->setOrder);
	free(pOrderedSet->setOrder);
	HashSet_destroy(pOrderedSet->collection);
	free(pOrderedSet->collection);
}

bool ListOrderedSet_add(ListOrderedSet * pOrderedSet, int object)
{
	if(HashSet_add(pOrderedSet->collection, object))
	{
		ArrayList_add1(pOrderedSet->setOrder, object);
		return true;
	}
	return false;
}

bool ListOrderedSet_remove(ListOrderedSet * pOrderedSet, int object)
{
	bool result = HashSet_remove(pOrderedSet->collection, object);
	ArrayList_remove(pOrderedSet->setOrder, object);
	return result;
}

bool ListOrderedSet_removeAll( ListOrderedSet * pOrderedSet,  ArrayList * coll)
{
	bool result = false;
	int i = 0;
	...
	{
		result |= ListOrderedSet_remove(pOrderedSet, coll->data[i]);
	}
...
int main(int argc, char ** argv)
{
	...
	ListOrderedSet * pOrderedSet = (ListOrderedSet *)malloc(sizeof(ListOrderedSet));
	ListOrderedSet_ListOrderedSet(pOrderedSet);

	for(i = 0; i < largeNumber; i ++)
	{
		ListOrderedSet_add(pOrderedSet, i);
	}


	ArrayList * toRemove = (ArrayList *)malloc(sizeof(ArrayList));
	ArrayList_ArrayList0(toRemove);

	for(i = largeNumber; i < 2 * largeNumber; i ++)
	{
		ArrayList_add1(toRemove, i);
	}

	ListOrderedSet_removeAll(pOrderedSet, toRemove);

	//ArrayList_print(pOrderedSet->setOrder);


	ArrayList_destroy(toRemove);
	free(toRemove);
	ListOrderedSet_ListOrderedSet(pOrderedSet);
	free(pOrderedSet);
}