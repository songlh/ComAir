...
bool SetUniqueList_retainAll(SetUniqueList * pList, ArrayList * coll)
{
	bool result = ArrayList_retainAll(pList->collection, coll);
	HashSet_retainAll(pList->set, coll);
	return result;
}
...
int main(int argc, char ** argv)
{
	...
	ArrayList * toRetain = (ArrayList *)malloc(sizeof(ArrayList));
	ArrayList_ArrayList0(toRetain);

	for(i = size; i < 2 * size ; i ++)
	{
		ArrayList_add1(toRetain, i);
	}

	SetUniqueList_retainAll(uniqueList, toRetain);
