...
WORK=$(pwd)
TEST_PROGRAM=${WORK}/test_two_for.c
BC_FILE=${WORK}/target.bc
MEM_FILE=${WORK}/target.mem.bc
...
do_not_sampling () {
 ...
 /HDD/llvm5.0/install/bin/clang -emit-llvm -c -Xclang -disable-O0-optnone -O0 -g -flto ${TEST_PROGRAM} -o ${BC_FILE}

 /HDD/llvm5.0/install/bin/opt -mem2reg ${BC_FILE} > ${MEM_FILE}

 /HDD/llvm5.0/install/bin/llvm-dis ${MEM_FILE}
