...
void MapEntry_MapEntry(MapEntry * pEntry, int h, int k, ArrayList * v, MapEntry * n)
{
	pEntry->value = v;
	pEntry->next = n;
	pEntry->key = k;
...
ArrayList * MapEntry_getValue(MapEntry * pEntry)
{
	return pEntry->value;
}

ArrayList * MapEntry_setValue(MapEntry * pEntry, ArrayList * value)
{
	ArrayList * oldValue = pEntry->value;
	pEntry->value = value;
	return oldValue;
}
...
ArrayList * HashMap_get(HashMap * pHash, int key)
{
	int hash = HashMap_hash(key);
	MapEntry * e = pHash->table[HashMap_indexFor(hash, pHash->capacity)];
...
void HashMap_addEntry(HashMap * pHash, int hash, int key, ArrayList * value, int bucketIndex)
{
...
bool HashMap_put(HashMap * pHash, int key, ArrayList * value)
{
	int hash = HashMap_hash(key);
...
typedef struct stMultiValueMap
{
	HashMap * map;
} MultiValueMap;

void MultiValueMap_MultiValueMap(MultiValueMap * pValueMap)
{
	pValueMap->map = (HashMap *)malloc(sizeof(HashMap));
	HashMap_HashMap0(pValueMap->map);
}

void MultiValueMap_destroy(MultiValueMap * pValueMap)
{
	HashMap_destroy(pValueMap->map);
}

ArrayList * MultiValueMap_getCollection(MultiValueMap * pValueMap, int key)
{
	return HashMap_get(pValueMap->map, key);
}

void MultiValueMap_put(MultiValueMap * pValueMap, int key, int value)
{
	bool result = false;
	ArrayList * coll = MultiValueMap_getCollection(pValueMap, key);

	if(coll == NULL)
	{
		coll = (ArrayList *)malloc(sizeof(ArrayList));
		ArrayList_ArrayList0(coll);
		ArrayList_add1(coll, value);
		HashMap_put(pValueMap->map, key, coll);
	}
	else
	{
		ArrayList_add1(coll, value);
...
bool contains(MultiValueMap * pValueMap, int k)
{
	int i = 0;
	int j = 0;
	MapEntry * p = NULL;
	...
	while(i <= pValueMap->map->capacity)
	{
		if(p == NULL && i < pValueMap->map->capacity)
		{
			p = pValueMap->map->table[i++];
			continue;
		}
...
bool containsAll(MultiValueMap * pValueMap, ArrayList * c)
{
	int i;
	for(i = 0; i < c->size; i ++)
	{
		if(!contains(pValueMap, c->data[i]))
		{
			printf("%d\n", c->data[i]);
			return false;
		}
...
int main(int argc, char ** argv)
{
	...
	int i, j;
	MultiValueMap * pValueMap = (MultiValueMap *)malloc(sizeof(MultiValueMap));
	MultiValueMap_MultiValueMap(pValueMap);

	for(i = 0; i < size; i ++)
	{
		MultiValueMap_put(pValueMap, i, i);
	}
	...
	bool bResult = containsAll(pValueMap, toContain);
	printf("%d\n", bResult);

	MultiValueMap_destroy(pValueMap);
	free(pValueMap);
