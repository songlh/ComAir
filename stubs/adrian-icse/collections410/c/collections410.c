...
#define DEFAULT_CAPACITY 400000
#define DEFAULT_INITIAL_CAPACITY 400000
...
void ArrayList_add2(ArrayList * pArray, int index, int element )
{
	...
	int * dst = pArray->data + pArray->size;
	int * src = pArray->data + pArray->size - 1;
	int count = pArray->size  - index;
...
typedef struct stSetUniqueList
{
	HashSet * set;
...
void SetUniqueList_SetUniqueList1(SetUniqueList * pList, ArrayList * coll)
{
	HashSet * temp = (HashSet *)malloc(sizeof(HashSet));
	HashSet_HashSet1(temp, coll);
	pList->set = temp;
	pList->collection = coll;
}

bool SetUniqueList_addAll(SetUniqueList * pList, int index, ArrayList * coll)
{
	HashSet * temp = (HashSet *)malloc(sizeof(HashSet));
	HashSet_HashSet1(temp, coll);
	HashSet_removeAll(temp, pList->collection);
	...
	{
		if(HashSet_contains(temp, coll->data[i]))
		{
			HashSet_add(pList->set, coll->data[i]);
			ArrayList_add2(pList->collection, index, coll->data[i]);
			index ++;
		}
	}

	HashSet_destroy(temp);
	free(temp);
	return true;
}


void SetUniqueList_destroy(SetUniqueList * pList)
{
	HashSet_destroy(pList->set);
	free(pList->set);
}
...
int main(int argc, char ** argv)
{
	...
	ArrayList * list = (ArrayList *)malloc(sizeof(ArrayList)); ArrayList_ArrayList0(list);
	int i = 0;
	...
	SetUniqueList * uniqueList = (SetUniqueList *)malloc(sizeof(SetUniqueList));
	SetUniqueList_SetUniqueList1(uniqueList, list);

	ArrayList * toAdd = (ArrayList *)malloc(sizeof(ArrayList));
	ArrayList_ArrayList0(toAdd);

	for(i = size -3; i < 2 * size + 1; i ++)
	{
		ArrayList_add1(toAdd, i);
	}

	SetUniqueList_addAll(uniqueList, 3, toAdd);

	//ArrayList_print(uniqueList->collection);
	//HashMap_print(uniqueList->set->map);
	SetUniqueList_destroy(uniqueList);
	free(uniqueList);
