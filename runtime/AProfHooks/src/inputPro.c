...
int pushStackElem(char *funcName, unsigned long count) {
    int expand;
    ...
    (psStack->StackElements[psStack->top]).cost = aprof_get_cost(); // get_cost() function;
    psStack->elementsNum++;
    psStack->top++;
    return 0;
...
void aprof_call(char *funcName) {
    ...
    push_result = pushStackElem(funcName, count);
    if (push_result != 0) {
