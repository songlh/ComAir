...
def run_command():
    for i in range(0, 20):
        ...
        num = generate_test_case(i)
        command = ['./target.aprof ', ' ', num, ' ', num]
...
def parse_log_file():
    ...
    df.to_csv('mozilla35294_result.csv', index=False)
...
def calculate_curve_fit():
    ...
    df = pd.read_csv('mozilla35294_result.csv')
    df = df.loc[df['func_id'] == 8]
