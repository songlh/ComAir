...
bool ListOrderedSet_contains(ListOrderedSet * pSet, int object)
{
	return HashSet_contains(pSet->collection, object);
}

bool ListOrderedSet_addAll(ListOrderedSet * pSet, int index, ArrayList * coll)
{
	bool changed = false;
	int i ;
	...
	{
		if(ListOrderedSet_contains(pSet, coll->data[i]))
		{
			continue;
		}

		HashSet_add(pSet->collection, coll->data[i]);
		ArrayList_add2(pSet->setOrder, index ++, coll->data[i]);
		changed = true;
	}
...
int main(int argc, char ** argv)
{
	...
	int i;
	ListOrderedSet * list = (ListOrderedSet *)malloc(sizeof(ListOrderedSet));
	ListOrderedSet_ListOrderedSet(list);

	for(i = 0; i < size; i ++)
	{
		ListOrderedSet_add(list, i);
	}
	...
	ListOrderedSet_addAll(list, 3, toAdd);
	ArrayList_print(list->setOrder);

	ListOrderedSet_destroy(list);
	free(list);
