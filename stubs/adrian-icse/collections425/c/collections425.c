...
typedef struct stListOrderedMap
{
	ArrayList * insertOrder;
	HashMap * map;

} ListOrderedMap;

void ListOrderedMap_ListOrderedMap(ListOrderedMap * pMap)
{
	pMap->insertOrder = (ArrayList *)malloc(sizeof(ArrayList));
	ArrayList_ArrayList0(pMap->insertOrder);

	pMap->map = (HashMap *)malloc(sizeof(HashMap));
	HashMap_HashMap0(pMap->map); 
}


void ListOrderedMap_destroy(ListOrderedMap * pMap)
{
	ArrayList_destroy(pMap->insertOrder);
	HashMap_destroy(pMap->map);
}

void ListOrderedMap_put(ListOrderedMap * pMap, int key, int value)
{
	if(HashMap_containsKey(pMap->map, key))
	{
		HashMap_put(pMap->map, key, value); 
	}
	else
	{
		HashMap_put(pMap->map, key, value);
		ArrayList_add1(pMap->insertOrder, key);
	}
}

void ListOrderedMap_remove(ListOrderedMap * pMap, int key)
{
	HashMap_remove(pMap->map, key);
	ArrayList_remove(pMap->insertOrder, key);
}

int main(int argc, char ** argv)
{
	int size = atoi(argv[1]);
	ListOrderedMap * map = (ListOrderedMap *)malloc(sizeof(ListOrderedMap));
	ListOrderedMap_ListOrderedMap(map);

	int i;

	for(i = 0; i < size; i ++ )
	{
		ListOrderedMap_put(map, i, i);
	}

	ArrayList * toRemoved = (ArrayList *)malloc(sizeof(ArrayList));
	ArrayList_ArrayList0(toRemoved);

	for(i = size; i < 2 * size; i ++)
	{
		ArrayList_add1(toRemoved, i);
	}

	for(i = 0; i < toRemoved->size; i ++ )
	{
		ListOrderedMap_remove(map, toRemoved->data[i]);
	}

	//ArrayList_print(map->insertOrder);

	ListOrderedMap_destroy(map);
	free(map);

	ArrayList_destroy(toRemoved);
	free(toRemoved);
}