...
bool IdentifyStack_containsAll(IdentifyStack * pStack, ArrayList * c )
{
	int i;
	for(i = 0; i < c->size; i ++ )
	{
		if(!IdentifyStack_contains(pStack, c->data[i]))
		{
			return false;
		}
	}
...
int main(int argc, char ** argv)
{
	...
	{
		ArrayList_add1(list, pStack->elementData[i]);
	}

	//ArrayList_print(list);
	bool bflag = IdentifyStack_containsAll(pStack, list);
