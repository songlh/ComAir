...
void IdentifyStack_removeElementAt(IdentifyStack * pStack, int index)
{
	...
	if(j > 0)
	{
		int * dst = pStack->elementData + index;
		int * src = pStack->elementData + index + 1;
		int count = pStack->elementCount - 1 - index;
...
bool IdentifyStack_retainAll(IdentifyStack * pStack, ArrayList * list)
{
	bool modified = false;
	int i, j ;

	for(i = 0; i < pStack->elementCount; i ++ )
	{
		if(!ArrayList_contains(list, pStack->elementData[i]) )
		{
			IdentifyStack_removeElementAt(pStack, i);
			i --;
			modified = true;
...
int main(int argc, char ** argv)
{
	...
	IdentifyStack * pStack = (IdentifyStack *)malloc(sizeof(IdentifyStack));
	IdentifyStack_IdentifyStack0(pStack);

	int i =0;
	for(i = 0; i < size; i ++ )
	{
		IdentifyStack_push(pStack, i);
	}

	//IdentifyStack_print(pStack);
	...
	IdentifyStack_retainAll(pStack, list);

	//IdentifyStack_print(pStack);

	IdentifyStack_destroy(pStack);
