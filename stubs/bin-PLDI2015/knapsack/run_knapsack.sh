...
WORK=$(pwd)
TEST_PROGRAM=${WORK}/knapsack-base.cpp
...
do_sampling () {
 ...
 /HDD/llvm5.0/install/bin/clang++ -c -g ${WORK}/getoptions.cpp -o ${WORK}/getoptions.o
...
/HDD/llvm5.0/install/bin/opt -load ${HOME}/lib/PrepareSampling/libPrepareSamplingPass.so -prepare-sampling  ${BC_FILE} > ${PRE_BC_FILE}
 ...
 # link c++
 /HDD/llvm5.0/install/bin/clang++ -O2 ${O_PROF_FILE} ${WORK}/getoptions.o ${RUNTIME_LIB} -lstdc++ -lm -lrt

 # test one time
 ./a.out -benchmark short
...
do_not_sampling () {
 ...
 # link c++
 /HDD/llvm5.0/install/bin/clang++ -O2 ${O_PROF_FILE} ${WORK}/getoptions.o ${RUNTIME_LIB}  -lm -lrt
