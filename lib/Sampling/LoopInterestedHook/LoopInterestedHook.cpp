...
std::set<Loop *> CollectSubLoop(Loop *ParentLoop) {
    ...
    while (workList.size() != 0) {
        ...
        if (loop != nullptr && !loop->empty()) {
            ...
            std::vector<Loop *> &subloopVect = ParentLoop->getSubLoopsVector();
...
void LoopInterestedHook::CollectInterestedLoopFunction() {

    std::map<Function *, std::set<Loop *>> InterestedFuncLoopMap;
    ...
    for (Module::iterator FI = this->pModule->begin(); FI != this->pModule->end(); FI++) {
        ...
        std::set<Loop *> FuncLoopSet;

        for (auto &lop: LoopInfo) {
            std::set<Loop *> TmpLoopSet = CollectSubLoop(lop);
            FuncLoopSet.insert(TmpLoopSet.begin(), TmpLoopSet.end());
        }

        InterestedFuncLoopMap[Func] = FuncLoopSet;
    }
...
void LoopInterestedHook::InstrumentCostUpdater(Function *pFunction, bool isOptimized) {
...
void LoopInterestedHook::InstrumentAlloc(Value *var, Instruction *BeforeInst) {
...
void LoopInterestedHook::InstrumentCallBefore(Function *F) {
 ...
 */
void LoopInterestedHook::InstrumentRmsUpdater(Function *F) {
...
void LoopInterestedHook::InstrumentRmsUpdater(Function *F, Instruction *BeforeInst) {
...
void LoopInterestedHook::ProcessMemIntrinsic(MemIntrinsic *memInst) {
...
/*
void LoopInterestedHook::InstrumentRmsUpdater(Function *pFunction) {
